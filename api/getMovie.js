...
exports.index = async function (ctx, next) {
	...
	let  type = qs.parse(query).type// 类型
	...
	let sql_type = type ? `and type="${type}" `: ''
	const sql = `select * from movie_data where name like "%${keyword}%" ${sql_type} ${sql_date} order by id asc limit ${pageNumber*pageSize}, ${pageSize}  `
	const sql2 = `select count(*) from movie_data where name like "%${keyword}%" ${sql_type} ${sql_date}`
